...
export class NotificationCron {
  ...
  async subscriptionEndingSoon(): Promise<void> {
    if (this.config.get<string>('NOTIFICATION_CRON_ENABLED') === 'false') return;
    try {
      const sent = await this.notifications.checkSubscriptionEndingSoon();
      if (sent > 0) {
        this.logger.log(`Subscription-ending notifications sent for ${sent} tenant(s)`);
      }
    } catch (err) {
  ...
  async weeklyDigest(): Promise<void> {
    if (this.config.get<string>('WEEKLY_DIGEST_CRON_ENABLED') === 'false') return;
    try {
